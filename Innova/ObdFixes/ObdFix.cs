...
{
    /// <summary>
    /// The ObdFix object handles the business logic and data access for the specialized business object, ObdFix.
    /// The class inherits from <see cref="BusinessObjectBase"/> class.
    /// </summary>
    /// <example>
    /// See the examples of how to construct the ObdFix object.
    ///
    /// To create a new instance of a new of ObdFix.
    /// <code>ObdFix o = (ObdFix)Registry.CreateInstance(typeof(ObdFix));</code>
    ///
    /// To create an new instance of an existing ObdFix.
    /// <code>ObdFix o = (ObdFix)Registry.CreateInstance(typeof(ObdFix), new Guid("{250426B4-3C7A-463c-93C4-045668C8A1AA}"));</code>
    /// </example>
    /// <remarks>
    /// In order to create an instance of ObdFix, you need to have a <see cref="Registry"/>
    ...
    /// </remarks>
    [Serializable()]
    public class ObdFix : InnovaBusinessObjectBase
    {
        private User user;
        ...
        private ObdFixStatus obdFixStatus;
        ...
        /// <summary>
        /// Constructor creates a new (one that does not exist, assigning a new <see cref="Guid"/> Id). ObdFix object.
        /// In order to create a new ObdFix which inherits from <see cref="BusinessObjectBase"/>, you need to use the <see cref="Registry"/>.
        /// </summary>
        /// <example>
        /// <code>
        /// ObdFix o = (ObdFix)Registry.CreateInstance(typeof(ObdFix));
        /// </code>
        /// </example>
        protected internal ObdFix() : base()
        {
        ...
        /// <summary>
        /// Constructor creates an existing (one that exists, and already has a <see cref="Guid"/> Id).  ObdFix object.
        /// In order to create an existing ObdFix object, which inherits from <see cref="BusinessObjectBase"/>, you need to use the <see cref="Registry"/>.
        /// </summary>
        /// <param name="id">The <see cref="Guid"/> or SQL uniqueidentifier indentification "Id" of the current object.</param>
        /// <example>
        /// <code>
        /// ObdFix o = (ObdFix)Registry.CreateInstance(typeof(ObdFix), new Guid("{250426B4-3C7A-463c-93C4-045668C8A1AA}"));
        /// </code>
        /// </example>
        protected internal ObdFix(Guid id) : base(id)
        {
            this.id = id;
        }
        ...
        /// <summary>
        /// Gets or sets the <see cref="User"/> that created the feedback.
        /// </summary>
        public User User
        {
            get
        ...
        /// <summary>
        /// Gets or sets the <see cref="DiagnosticReportFixFeedback"/> the feedback is associated with.
        /// </summary>
        ...
        /// DEPRICATED - Use the DiagnosticReport instead.
        /// </summary>
        private string MilDtc
        {
            get
        ...
        /// <summary>
        /// Gets or sets the <see cref="Fix"/> that was selected for the feedback.
        /// </summary>
        public Fix Fix
        {
            get
        ...
        /// <summary>
        /// Gets or sets the <see cref="FixName"/> that was selected for the feedback.
        /// </summary>
        ...
        /// </summary>
        [PropertyDefinition("Fix Name", "Fix Submitted by Technician", "Fix Submitted by Technician", "The name of the fix.")]
        public string Name
        {
            get
            {
        ...
        /// <summary>
        /// Gets or sets the <see cref="ObdFixStatus"/> status of the feedback record.
        /// </summary>
        public ObdFixStatus ObdFixStatus
        {
            get
            {
                EnsureLoaded();
        ...
        /// <summary>
        /// Gets or sets the <see cref="DateTime"/> the feedback was created in universal time.
        /// </summary>
		...
		*******************************************************************************************/
        ...
        public new SqlTransaction Load(SqlConnection connection, SqlTransaction transaction, bool isLoadBase)
        {
            ...
            if (isLoadBase)
            {
                //load the base obdFix if user selected it.
        ...
        {
            dr.ProcedureName = "ObdFix_Load";
            dr.AddGuid("ObdFixId", Id);
        }
        ...
        protected new void SetPropertiesFromDataReader(SqlDataReaderWrapper dr)
        {
            ...
            this.diagnosticReportFixFeedback = (DiagnosticReportFixFeedback)Registry.CreateInstance(typeof(DiagnosticReportFixFeedback), dr.GetGuid("DiagnosticReportFixFeedbackId"));
            this.fix = (Fix)dr.GetBusinessObjectBase(Registry, typeof(Fix), "FixId");
            ...
            this.obdFixStatus = (ObdFixStatus)dr.GetInt32("ObdFixStatus");
        ...
        public override SqlTransaction Save(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            if (IsObjectDirty)
            {
                ...
                using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
                {
                    if (IsObjectCreated)
                    {
                        dr.ProcedureName = "ObdFix_Create";
                        CreatedDateTimeUTC = DateTime.UtcNow;
                    }
                    else
                    {
                        dr.ProcedureName = "ObdFix_Save";
                    }
                    ...
                    //
                    dr.AddBusinessObject("FixId", Fix);
                    dr.AddBusinessObject("FixNameId", FixName);
                    dr.AddNVarChar("MilDtc", MilDtc);
                    ...
                    dr.AddInt32("ObdFixStatus", (int)ObdFixStatus);
        ...
        {
            transaction = this.DiagnosticReportFixFeedback.Save(connection, transaction);
            transaction = this.User.Save(connection, transaction);
        ...
        public override SqlTransaction Delete(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            /* Example of deleting the obdFixs
			 ...
			 *  child related obdFixs. See example below).
			 *
			// delete the child related objects
			foreach (ObdFixChild obdFixChild in ObdFixChilds)
			{
				transaction = obdFixChild.Delete(connection, transaction);
			}
			*/
            ...
            using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
            {
                ...
                /*
				dr.ProcedureName = "ObdFixsOthers_Delete";
				dr.AddGuid("ObdFixId", Id);
				...
				*/
                ...
                //delete the obdFix
                dr.ProcedureName = "ObdFix_Delete";
                dr.AddGuid("ObdFixId", Id);
