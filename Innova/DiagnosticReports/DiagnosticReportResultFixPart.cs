...
namespace Innova.DiagnosticReports
{
    ...
    /// </remarks>
    [Serializable()]
    public class DiagnosticReportResultFixPart : InnovaBusinessObjectBase
    {
        ...
        private DiagnosticReportResultFixPartPartOemCollection diagnosticReportResultFixPartPartOems;
        ...
        /// </code>
        /// </example>
        protected internal DiagnosticReportResultFixPart() : base()
        {
        ...
        /// </code>
        /// </example>
        protected internal DiagnosticReportResultFixPart(Guid id) : base(id)
        {
            this.id = id;
        }
        ...
        //#ValveTask
        /// <summary>
        /// Gets or sets the <see cref="PartName"/>
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="NullableDecimal"/> price in the currency set on the <see cref="DiagnosticReport"/>.
        /// </summary>
        public NullableDecimal PriceInLocalCurrency
        {
            ...
            {
                this.EnsureLoaded();
                if (this.priceInLocalCurrency != value)
                {
                    this.IsObjectDirty = true;
                    this.priceInLocalCurrency = value;
                }
            }
        }

        /// <summary>
        /// Gets the <see cref="decimal"/> price to be presented based on the currenct currency in the <see cref="RuntimeInfo"/>.
        /// </summary>
        public decimal Price_Presented
        {
            get
            {
                return this.DiagnosticReport.GetCurrencyValueForPresentration(this.Price, this.PriceInLocalCurrency);
            }
        }
        ...
        //#ValveTask
        /// <summary>
        /// Gets or sets the <see cref="string"/> ACESId.
        /// </summary>
        ...
        private DiagnosticReport DiagnosticReport
        {
            get
            {
                return this.DiagnosticReportResultFix.DiagnosticReportResult.DiagnosticReport;
            }
        }
		...
		*******************************************************************************************/
        ...
        /// <summary>
        /// Gets the <see cref="DiagnosticReportResultFixPartPartOemCollection"/> for the DiagnosticReportResultFix
        /// </summary>
        public DiagnosticReportResultFixPartPartOemCollection DiagnosticReportResultFixPartPartOems
        {
            get
            {
                if (diagnosticReportResultFixPartPartOems == null)
                {
                    diagnosticReportResultFixPartPartOems = new DiagnosticReportResultFixPartPartOemCollection(Registry);

                    if (!isObjectCreated)
                    {
                        SqlProcedureCommand call = new SqlProcedureCommand();

                        call.ProcedureName = "DiagnosticReportResultFixPartPartOem_LoadByXmlGuidList";
                        call.AddNText("XmlGuidList", "<xmlguidlist>" + Id.ToString() + "</xmlguidlist>");
                        diagnosticReportResultFixPartPartOems.Load(call, "DiagnosticReportResultFixPartPartOemId");
                    }
                }

                return diagnosticReportResultFixPartPartOems;
            }
        }
        ...
        /// <summary>
        /// Sets the local currency values on the part.
        /// </summary>
        protected internal void SetLocalCurrencyValues()
        {
        ...
        {
            dr.ProcedureName = "DiagnosticReportResultFixPart_Load";
        ...
        protected new void SetPropertiesFromDataReader(SqlDataReaderWrapper dr)
        {
            ...
            if (!dr.IsDBNull("PartId"))
            {
                this.part = (Part)this.Registry.CreateInstance(typeof(Part), dr.GetGuid("PartId"));
            }

            //#ValveTask
            if (dr.ColumnExists("PartNameId") && !dr.IsDBNull("PartNameId"))
            {
                this.partName = (PartName)this.Registry.CreateInstance(typeof(PartName), dr.GetGuid("PartNameId"));
            }

            this.makesList = dr.GetString("MakesList");
            this.partNumber = dr.GetString("PartNumber");
            ...
            this.priceInLocalCurrency = dr.GetNullableDecimal("PriceInLocalCurrency");
        ...
        public override SqlTransaction Save(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            if (IsObjectDirty)
            {
                ...
                using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
                {
                    ...
                    {
                        dr.ProcedureName = "DiagnosticReportResultFixPart_Create";
                    }
                    else
                    {
                        dr.ProcedureName = "DiagnosticReportResultFixPart_Save";
                    }

                    dr.AddGuid("DiagnosticReportResultFixPartId", this.Id);
                    ...
                    {
                        dr.AddGuid("PartNameId", this.PartName.Id);
                    }

                    dr.AddNVarChar("MakesList", this.MakesList);
                    dr.AddNVarChar("PartNumber", this.PartNumber);
                    ...
                    if (this.PriceInLocalCurrency.HasValue)
                    {
                        dr.AddDecimal("PriceInLocalCurrency", Math.Round(this.PriceInLocalCurrency.Value, 2));
                    }
        ...
        protected new SqlTransaction SaveCollections(SqlConnection connection, SqlTransaction transaction)
        {
            if (DiagnosticReportResultFixPartPartOems != null)
            {
                for (int i = 0; i < DiagnosticReportResultFixPartPartOems.Removed.Count; i++)
                {
                    transaction = ((DiagnosticReportResultFixPartPartOem)DiagnosticReportResultFixPartPartOems.Removed[i]).Delete(connection, transaction);
                }
                for (int i = 0; i < DiagnosticReportResultFixPartPartOems.Count; i++)
                {
                    transaction = DiagnosticReportResultFixPartPartOems[i].Save(connection, transaction);
                }
            }
			...
			*/
            ...
            using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
            {
				...
				*/
                ...
                dr.ProcedureName = "DiagnosticReportResultFixPart_Delete";
                dr.AddGuid("DiagnosticReportResultFixPartId", this.Id);
