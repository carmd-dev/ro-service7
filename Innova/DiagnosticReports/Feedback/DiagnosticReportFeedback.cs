...
{
    /// <summary>
    /// The DiagnosticReportFeedback object handles the business logic and data access for the specialized business object, DiagnosticReportFeedback.
    /// The class inherits from <see cref="BusinessObjectBase"/> class.
    /// </summary>
    /// <example>
    /// See the examples of how to construct the DiagnosticReportFeedback object.
    ///
    /// To create a new instance of a new of DiagnosticReportFeedback.
    /// <code>DiagnosticReportFeedback o = (DiagnosticReportFeedback)Registry.CreateInstance(typeof(DiagnosticReportFeedback));</code>
    ///
    /// To create an new instance of an existing DiagnosticReportFeedback.
    /// <code>DiagnosticReportFeedback o = (DiagnosticReportFeedback)Registry.CreateInstance(typeof(DiagnosticReportFeedback), new Guid("{250426B4-3C7A-463c-93C4-045668C8A1AA}"));</code>
    /// </example>
    /// <remarks>
    /// In order to create an instance of DiagnosticReportFeedback, you need to have a <see cref="Registry"/>
    ...
    /// </remarks>
    [Serializable()]
    public class DiagnosticReportFeedback : BusinessObjectBase
    {
        private DiagnosticReport diagnosticReport;
        private Fix fix;
        private Guid uploadedFilesFolderId = Guid.Empty;
        private RepairedByType repairedByType;
        private DateTime createdDateTimeUTC;
        private string comments;

        private DiagnosticReportFeedbackLaborCollection laborRecords;
        private DiagnosticReportFeedbackPartCollection partRecords;
        ...
        /// <summary>
        /// Constructor creates a new (one that does not exist, assigning a new <see cref="Guid"/> Id). DiagnosticReportFeedback object.
        /// In order to create a new DiagnosticReportFeedback which inherits from <see cref="BusinessObjectBase"/>, you need to use the <see cref="Registry"/>.
        /// </summary>
        /// <example>
        /// <code>
        /// DiagnosticReportFeedback o = (DiagnosticReportFeedback)Registry.CreateInstance(typeof(DiagnosticReportFeedback));
        /// </code>
        /// </example>
        protected internal DiagnosticReportFeedback() : base()
        {
        ...
        /// <summary>
        /// Constructor creates an existing (one that exists, and already has a <see cref="Guid"/> Id).  DiagnosticReportFeedback object.
        /// In order to create an existing DiagnosticReportFeedback object, which inherits from <see cref="BusinessObjectBase"/>, you need to use the <see cref="Registry"/>.
        /// </summary>
        /// <param name="id">The <see cref="Guid"/> or SQL uniqueidentifier indentification "Id" of the current object.</param>
        /// <example>
        /// <code>
        /// DiagnosticReportFeedback o = (DiagnosticReportFeedback)Registry.CreateInstance(typeof(DiagnosticReportFeedback), new Guid("{250426B4-3C7A-463c-93C4-045668C8A1AA}"));
        /// </code>
        /// </example>
        protected internal DiagnosticReportFeedback(Guid id) : base(id)
        {
            this.id = id;
        }
        ...
        /// </summary>
        public Guid UploadedFilesFolderId
        {
            get
            {
                EnsureLoaded();
                return uploadedFilesFolderId;
            }
            set
            {
                EnsureLoaded();
                if (uploadedFilesFolderId != value)
                {
                    IsObjectDirty = true;
                    uploadedFilesFolderId = value;
                }
            }
        }
        ...
        /// </summary>
        public RepairedByType RepairedByType
        {
            get
            {
                EnsureLoaded();
                return repairedByType;
            }
            set
            {
                EnsureLoaded();
                if (repairedByType != value)
                {
                    IsObjectDirty = true;
                    repairedByType = value;
                }
            }
        }
		...
		*******************************************************************************************/
        ...
        /// <summary>
        /// Gets a <see cref="DiagnosticReportFeedbackLaborCollection"/> of <see cref="DiagnosticReportFeedbackLabor"/> objects for this report.
        /// </summary>
        public DiagnosticReportFeedbackLaborCollection LaborRecords
        {
            get
            {
                if (laborRecords == null)
                {
                    laborRecords = new DiagnosticReportFeedbackLaborCollection(Registry);
                    ...
                    if (!isObjectCreated)
                    {
                        ...
                        call.ProcedureName = "DiagnosticReportFeedbackLabor_LoadByDiagnosticReportFeedback";
                        call.AddGuid("DiagnosticReportFeedbackId", Id);

                        laborRecords.Load(call, "DiagnosticReportFeedbackLaborId", true, true);
                    }
                }
        ...
        /// <summary>
        /// Gets a <see cref="DiagnosticReportFeedbackPartCollection"/> of <see cref="DiagnosticReportFeedbackPart"/> objects for this report.
        /// </summary>
        public DiagnosticReportFeedbackPartCollection PartRecords
        {
            get
            {
                if (partRecords == null)
                {
                    partRecords = new DiagnosticReportFeedbackPartCollection(Registry);
                    ...
                    if (!isObjectCreated)
                    {
                        ...
                        call.ProcedureName = "DiagnosticReportFeedbackPart_LoadByDiagnosticReportFeedback";
                        call.AddGuid("DiagnosticReportFeedbackId", Id);

                        partRecords.Load(call, "DiagnosticReportFeedbackPartId", true, true);
                    }
                }
        ...
        public new SqlTransaction Load(SqlConnection connection, SqlTransaction transaction, bool isLoadBase)
        {
            ...
            if (isLoadBase)
            {
                //load the base diagnosticReportFeedback if user selected it.
        ...
        {
            dr.ProcedureName = "DiagnosticReportFeedback_Load";
            dr.AddGuid("DiagnosticReportFeedbackId", Id);
        }
        ...
        {
            diagnosticReport = (DiagnosticReport)Registry.CreateInstance(typeof(DiagnosticReport), dr.GetGuid("DiagnosticReportId"));
            if (!dr.IsDBNull("FixId"))
            {
                fix = (Fix)Registry.CreateInstance(typeof(Fix), dr.GetGuid("FixId"));
            }
            if (!dr.IsDBNull("UploadedFilesFolderId"))
            {
                uploadedFilesFolderId = dr.GetGuid("UploadedFilesFolderId");
            }
            repairedByType = (RepairedByType)dr.GetInt32("RepairedByType");
        ...
        public override SqlTransaction Save(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            if (IsObjectDirty)
            {
                ...
                using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
                {
                    if (IsObjectCreated)
                    {
                        dr.ProcedureName = "DiagnosticReportFeedback_Create";
                        CreatedDateTimeUTC = DateTime.UtcNow;
                    }
                    else
                    {
                        dr.ProcedureName = "DiagnosticReportFeedback_Save";
                    }
                    ...
                    if (UploadedFilesFolderId != Guid.Empty)
                    {
                        dr.AddGuid("UploadedFilesFolderId", UploadedFilesFolderId);
                    }
                    dr.AddInt32("RepairedByType", (int)RepairedByType);
                    dr.AddNText("Comments", Comments);
        ...
        {
            // Save the LaborRecords collection
            if (laborRecords != null)
            {
                foreach (DiagnosticReportFeedbackLabor lr in LaborRecords.Removed)
                {
                    transaction = lr.Delete(connection, transaction);
                }

                foreach (DiagnosticReportFeedbackLabor lr in LaborRecords)
                {
                    transaction = lr.Save(connection, transaction);
                }
            }

            // Save the PartRecords collection
            if (partRecords != null)
            {
                foreach (DiagnosticReportFeedbackPart pr in PartRecords.Removed)
                {
                    transaction = pr.Delete(connection, transaction);
                }

                foreach (DiagnosticReportFeedbackPart pr in PartRecords)
                {
                    transaction = pr.Save(connection, transaction);
                }
            }
        ...
        public override SqlTransaction Delete(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            using (SqlDataReaderWrapper dr = new SqlDataReaderWrapper(connection, false))
            {
                //delete the diagnosticReportFeedback
                dr.ProcedureName = "DiagnosticReportFeedback_Delete";
                dr.AddGuid("DiagnosticReportFeedbackId", Id);
